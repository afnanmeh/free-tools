...
import { Container, Title, Text, Button, SimpleGrid, Card, Box, TextInput, Paper } from '@mantine/core';
import Link from 'next/link';
import { SiteHeader } from '@/components/layout/SiteHeader';
import { MinimalCanvas } from '@/components/AnimatedCanvas';
import { IconCode, IconSearch, IconPalette, IconCalculator, IconSettings } from '@tabler/icons-react';
      ...
      <Container size="xl">
        <div className={styles.heroContent}>
          ...
          <Text className={styles.heroDescription}>
            From Development to Marketing: Powerful utilities for developers, marketers, and designers
          </Text>
          ...
          <div style={{ margin: '2rem auto 0', width: '100%', maxWidth: 720 }}>
            <div style={{ position: 'relative' }}>
              <TextInput
                ...
                value={query}
                onChange={(e) => { setQuery(e.currentTarget.value); setHighlight(-1); }}
                aria-label="Search tools"
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    if (highlight >= 0) goToIndex(highlight); else goToFirstResult();
                ...
                styles={{
                  input: {
                    ...
                    boxShadow: '0 8px 30px rgba(0,0,0,0.45), 0 0 0 1px rgba(245,158,11,0.18)',
                    color: '#fff',
                  },
                }}
                rightSection={
                  <Button
                    aria-label="Search"
                    size="sm"
                    onClick={() => (highlight >= 0 ? goToIndex(highlight) : goToFirstResult())}
                    styles={{
                      root: {
                ...
                >
                  {results.map((tool, i) => (
                    <button
                      ...
                      onMouseLeave={() => setHoveredId((id) => (id === tool.id ? null : id))}
                      style={{
                        display: 'block',
                        width: '100%',
                        textAlign: 'left',
                        ...
                        padding: '12px 14px',
                        cursor: 'pointer',
                        borderLeft: (highlight === i || hoveredId === tool.id) ? '3px solid #F59E0B' : '3px solid transparent',
                        background: (highlight === i || hoveredId === tool.id) ? 'rgba(245,158,11,0.08)' : 'transparent',
                      }}
                    >
                      <div style={{ color: '#fff', fontWeight: 700, fontSize: 14 }}>{tool.name}</div>
                      <div style={{ color: '#aaa', fontSize: 12 }}>{tool.description}</div>
                    </button>
                  ))}
                </Paper>
              )}
...
function FeatureSection({ eyebrow, title, description, features, reverse, accentColor }: FeatureSectionProps) {
  ...
  return (
    <section className={styles.featureSection} ref={sectionRef} aria-labelledby={`feature-${eyebrow.toLowerCase().replace(/\s+/g, '-')}`}>
      <Container size="xl">
        <div className={`${styles.featureBox} ${reverse ? styles.reverse : ''} ${
          eyebrow === 'DEVELOPER TOOLS' ? '' :
          eyebrow === 'SEO & MARKETING' ? styles.seo :
          eyebrow === 'DESIGN TOOLS' ? styles.design :
          eyebrow === 'CALCULATORS' ? styles.calculators : ''
        }`}>
          <div className={`${styles.featureGrid} ${reverse ? styles.reverse : ''}`}>
            <div className={styles.featureContent}>
              <Text className={styles.eyebrow} style={{ color: accentColor }}>{eyebrow}</Text>
              <h2 className={styles.featureTitle} id={`feature-${eyebrow.toLowerCase().replace(/\s+/g, '-')}`}>{title}</h2>
              <Text className={styles.featureDescription}>{description}</Text>
              <ul className={styles.featureList}>
                {features.map((feature, index) => (
                  <li key={index} className={styles.featureItem}>
                    <span className={styles.checkmark} style={{ backgroundColor: accentColor }}>✓</span>
                    {feature}
                  </li>
                ))}
              </ul>
            ...
            {eyebrow === 'DEVELOPER TOOLS' && (
              <div className={styles.mockDashboard}>
                <div className={styles.dashboardHeader}>
                  <div className={styles.dashboardTab} style={{ backgroundColor: accentColor, color: '#fff' }}>JSON Formatter</div>
                  <div className={styles.dashboardTab}>Output</div>
                </div>
                <div className={styles.dashboardContent}>
                  <div className={styles.codeLine}>{'{'}</div>
                  <div className={styles.codeLine}>  "name": "John Doe",</div>
                  <div className={styles.codeLine}>  "email": "john@example.com",</div>
            ...
            {eyebrow === 'SEO & MARKETING' && (
              <div className={styles.mockDashboard}>
                <div className={styles.dashboardHeader}>
                  <div className={styles.dashboardTab} style={{ backgroundColor: accentColor, color: '#fff' }}>Meta Tags</div>
                  <div className={styles.dashboardTab}>Preview</div>
                </div>
                <div className={styles.dashboardContent}>
                  <div className={styles.metaPreview}>
                    <div className={styles.metaTitle}>Your Page Title Here</div>
            ...
            {eyebrow === 'DESIGN TOOLS' && (
              <div className={styles.colorPalette}>
                ...
                <div className={styles.colorSwatch} style={{ backgroundColor: '#EC4899' }}></div>
                <div className={styles.colorSwatch} style={{ backgroundColor: '#8B5CF6' }}></div>
                <div className={styles.colorSwatch} style={{ backgroundColor: '#10b981' }}></div>
                <div className={styles.colorSwatch} style={{ backgroundColor: '#06b6d4' }}></div>
              </div>
            )}
            {eyebrow === 'CALCULATORS' && (
              <div className={styles.mockDashboard}>
                <div className={styles.dashboardHeader}>
                  <div className={styles.dashboardTab} style={{ backgroundColor: accentColor, color: '#fff' }}>SaaS Metrics</div>
...
function CategoriesSection() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const cards = entry.target.querySelectorAll(`.${styles.categoryCard}`);
  ...
  const categories = [
    {
      ...
      description: 'JSON formatters, JWT decoders, regex testers, and code utilities',
  ...
  return (
    <section className={styles.categoriesSection} ref={sectionRef} aria-labelledby="categories-heading">
      <Container size="xl">
        <div className={styles.sectionHeader}>
          <h2 className={styles.sectionTitle} id="categories-heading">Explore by Category</h2>
        ...
        <SimpleGrid cols={{ base: 1, sm: 2, md: 4 }} spacing="xl">
          {categories.map((category, index) => (
            <Link key={index} href={category.link} style={{ textDecoration: 'none' }}>
              <Card className={styles.categoryCard} style={{ '--accent-color': category.color } as React.CSSProperties}>
                <div className={styles.categoryIcon} style={{ backgroundColor: `${category.color}20`, color: category.color }} aria-hidden="true">
                  {category.icon}
                </div>
                <h3 className={styles.categoryTitle}>{category.title}</h3>
                <Text className={styles.categoryDescription}>{category.description}</Text>
                <div className={styles.categoryCount} style={{ color: category.color }}>
...
function CTASection() {
  ...
  return (
    ...
    <section className={styles.ctaSection} ref={sectionRef} aria-labelledby="cta-heading">
        <div className={styles.ctaGearLeft} aria-hidden="true">
          <IconSettings size={180} color="white" className={styles.rotatingGearReverse} />
        </div>
        ...
        <div className={styles.ctaGear} aria-hidden="true">
          <IconSettings size={420} color="white" className={styles.rotatingGear} />
        </div>
  ...
  const testimonials = [
    {
      ...
      company: "TechCorp",
      content: "These tools have saved me hours of work every week. The JSON formatter alone is worth its weight in gold."
    },
    {
      name: "Michael Rodriguez",
      role: "Marketing Manager", 
      company: "Growth Labs",
      content: "The SEO tools are incredibly comprehensive. I can generate meta tags and test them all in one place."
    },
    {
      name: "Emily Watson",
      role: "UX Designer",
      company: "Design Studio",
      content: "The color tools help me ensure accessibility while creating beautiful palettes. Essential for my workflow."
    },
    {
      name: "David Kim",
      role: "SaaS Founder",
      company: "StartupHub",
      content: "The calculators helped me understand my metrics better. No more complex spreadsheets needed!"
    },
    {
      name: "Lisa Anderson",
      role: "Full Stack Developer",
      company: "DevWorks",
      content: "JWT decoder and regex tester are my daily companions. Fast, secure, and reliable."
    },
    {
      name: "James Taylor",
      role: "Content Strategist",
      company: "MediaCo",
      content: "The readability checker improved our content quality significantly. Our engagement has never been better."
    }
  ];
    ...
    <section className={styles.testimonialsSection} aria-labelledby="testimonials-heading">
      <Container size="xl">
        <div className={styles.testimonialsHeader}>
          <div className={styles.testimonialsBadge}>Trusted by Professionals</div>
          <h2 className={styles.testimonialsTitle} id="testimonials-heading">
            Loved by <span className={styles.titleAccent}>10,000+</span> users worldwide
          </h2>
          <Text className={styles.testimonialsDescription}>
            From developers to marketers, designers to founders - professionals everywhere rely on our tools
          </Text>
        </div>
  ...
  const faqs = [
    {
      ...
      answer: "Yes! All our tools are completely free to use. No signup required, no hidden fees, no premium tiers. Just powerful tools that work."
    },
    {
      question: "Is my data secure?",
      answer: "Absolutely. All processing happens directly in your browser using JavaScript. Your data never leaves your device, ensuring complete privacy and security."
    },
    {
      question: "Can I use these tools commercially?",
      answer: "Yes, all tools can be used for both personal and commercial projects. We don't place any restrictions on how you use the output from our tools."
    },
    {
      question: "Do I need to create an account?",
      answer: "No account required. Just visit the tool page and start using it immediately. We believe in frictionless access to useful utilities."
    },
    {
      question: "Are there any API limits?",
      answer: "Since everything runs in your browser, there are no API limits or rate restrictions. Use the tools as much as you need, whenever you need them."
    },
    {
      question: "Can I suggest new tools?",
      answer: "Absolutely! We're always looking to expand our collection. Send us your suggestions through our contact page and we'll consider adding them."
    }
  ];
          ...
          {faqs.map((faq, index) => (
            ...
            >
              <div className={styles.faqQuestion}>
                <h3>{faq.question}</h3>
                <div className={`${styles.faqToggle} ${activeIndex === index ? styles.open : ''}`}>
                  <span></span>
                  <span></span>
                </div>
              </div>
